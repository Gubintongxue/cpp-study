# this指针那些事[¶](https://light-city.github.io/basic_content/this//#this "Permanent link")
...
![](image/thiscontrust-16858907054974.png)

![](image/genthis-16858907117756.png)

会发现编译器自动为我们加上`A* const`，而不是`A const *this`！

紧接着，上述还有个常函数，那么我们在对`get_age`添加断点，如下：

![](image/constthis-16858907412108.png)
...
总结：this在成员函数的开始执行前构造，在成员的执行结束后清除。上述的get\_age函数会被解析成`get_age(const A * const this)`,`add_age`函数会被解析成`add_age(A* const this,int a)`。在C++中类和结构是只有一个区别的：类的成员默认是private，而结构是public。this是类的指针，如果换成结构，那this就是结构的指针了。